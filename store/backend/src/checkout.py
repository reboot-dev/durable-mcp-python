...
class CheckoutServicer(Checkout.Servicer):
    ...
    async def place_order(
        ...
        cart_ref = Cart.ref("default-cart")
        cart_response = await cart_ref.get_items(context)
        ...
        order = store_pb2.Order(
            order_id=order_id, items=order_items,
            shipping_cost_cents=shipping_cost_cents,
            shipping_address=request.shipping_address, status="PENDING",
            created_at=int(time.time() * 1000), tracking_number=""
        )
        ...
        # Empty the cart
        await cart_ref.empty_cart(context, store_pb2.EmptyCartRequest())
